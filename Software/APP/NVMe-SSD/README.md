...
* **多传感器数据存储**：支持摄像头（NV12）、激光雷达（带帧头时间戳）、IMU 三种数据类型
* **视频回溯导出**：从 NVMe 原始记录扫描并导出为 MP4 视频（MPP 硬件编码）
* **雷达热力图导出**：回溯窗口内 LiDAR 点云渲染为 PNG 热力图（时间→色相映射），带标题栏信息
* **累积缓冲优化**：LiDAR/IMU 小数据量累积批量写盘，flush() 确保回溯前数据落盘
