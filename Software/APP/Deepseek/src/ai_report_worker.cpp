...
AIReportWorker::~AIReportWorker()
{
    stop();
}

void AIReportWorker::setConfig(const DeepSeekInference::Config& cfg)
{
    config_ = cfg;
}
...
void AIReportWorker::start()
{
    ...
    while (running_.load()) {
        ...
        QThread::msleep(200);  // 200ms 轮询间隔，响应延迟可接受
    }
