...
### 5.2 簇→bbox认领 (评分制 + 相机感知 + 深度仲裁)

每个有效簇, 投影质心到每个相机, 对**同相机**的bbox计算认领得分:
...
- **相机感知**: bbox只能匹配同相机的投影。`bboxCamIdx` 数组标识每个bbox的来源相机, 投影循环跳过 `bboxCamIdx[bb] != cc` 的组合, 避免 cam0 簇被 cam1 bbox 误认领
- `proximityFactor ≤ 0` 时该bbox不认领此簇(投影点太远)
- 每个bbox选择得分最高的簇 → 产出 **bbox检测** (`isOrphan=false`)
- 一簇最多被一个bbox认领（赢家通吃+补选）

**深度优先仲裁**: 冲突解决后, 比较每个bbox的得分最高簇和距离最近簇。若最近簇比得分最高簇近 ≥ 0.5m 且未被其他bbox认领, 则强制选最近簇。解决"人椅一前一后"场景中椅子簇因投影恰在框内而抢走人的bbox的问题。日志标记 `depth-wins`。

**设计意图**: 当人在图像边沿时, LiDAR质心可能投影到框外(特别是框不完全覆盖人体时)。旧方案要求质心必须在框内 → 边沿目标丢失。新方案对"框外但邻近"的簇仍给予评分 → 边沿目标也能认领到簇。深度仲裁确保同bbox候选簇中距离最近的优先。
