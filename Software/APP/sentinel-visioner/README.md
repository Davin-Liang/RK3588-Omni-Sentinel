...
* **双类型相机支持** ：统一 API 同时管理 MIPI CSI（RK3588 ISP）和 USB UVC 两种不同类型相机，通过 `CameraType` 枚举区分。USB 相机自动协商 NV12/YUYV 像素格式，YUYV 时由 RGA 硬件转为 NV12 再送入统一下游管线。
...
以下数据基于 RK3568/RK3588 平台实测，记录了在  **双路异步队列满载运行** （1080P输入 -> NPU 640x640分支 + 1080P RGB888 预览分支 + 1080P推流拷贝分支）下的极高并发性能：
