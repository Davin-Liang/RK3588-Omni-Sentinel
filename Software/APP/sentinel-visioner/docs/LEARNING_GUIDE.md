...
### 决策 7：为什么 NPU 和预览 buffer 处理互不阻塞？

**问题**: 最初 NPU 和预览 buffer 的获取和 RGA 处理共用一个 `if (targetNpuBuf != nullptr)` 门控。NPU 池无人消费 8 帧后枯竭 → 预览跟随停摆。

**方案**: 拆为两个独立 `if` 块，各自的 buffer 池互不干扰。NPU 池空只影响 NPU，预览照常。

| 对比维度 | 共用一个门控（旧方案） | 独立处理（新方案） |
|----------|---------------------|-------------------|
| NPU 池空 | 预览也停 | 预览不受影响 |
| 预览池空 | NPU 照常 | NPU 照常 |
| 代码复杂度 | 略低 | 多一个 if 块 |

**面试话术**: "NPU 推理暂时没有消费者，但我们不希望因为这影响到预览管线。把 NPU 和预览的 buffer 分配和 RGA 处理拆成独立分支，各自的池子各自管理——这是典型的'故障隔离'模式。"

---

### 决策 8：为什么用 FFmpeg 软件解码 MJPG 而不是硬件 JPEG 解码器？

**需求**: USB 相机 MJPG 格式支持 1080p@30fps（YUYV 1080p 仅 6fps）。

**方案**: 使用项目已有的 FFmpeg `libavcodec` MJPEG 解码器，软件解码后得到 YUV 平面数据，打包为 NV12 送入统一下游管线。

| 对比维度 | RK3588 VPU 硬件 JPEG | FFmpeg 软件解码（我们的方案） |
|----------|---------------------|---------------------------|
| 接入成本 | 需适配 V4L2 M2M 接口，新增 DMA buffer 管理 | 已有 FFmpeg 依赖，加一个解码函数即可 |
| 解码延迟 | ~1ms | 3-5ms（1080p 单帧） |
| 30fps 可行性 | 轻松 | 完全够用（33ms 间隔） |
| 格式适配 | 固定 | 自动适配 420/422 子采样 |

**面试话术**: "RK3588 有硬件 JPEG 解码器，但接入成本高——要新增 V4L2 M2M 设备的 DMA buffer 管理。FFmpeg 软件解码单帧 3-5ms，1080p 30fps 的 33ms 间隔完全够用。而且我们项目已经依赖了 FFmpeg，不引入新依赖。"
