...
* **双相机参数化配置**：新增 `ImuConfig` 和 `EisCameraConfig`，将 IMU 全局参数与每路相机 EIS 参数分离，支持 15FPS/30FPS 双相机分别设置 `halfWindowMs`、`focalX/focalY`、`timeOffsetMs`、`signX/signY`、`maxOffsetPixel` 和 `smoothingAlpha`。
...
│   ├── JITTER-EVALUATION.md       # raw/eis视频抖动RMS评估说明
│   └── PARAMETER-SELECTION.md     # 双相机EIS防抖参数选取说明
├── include/                      # 头文件目录
...
其中 `EisCameraConfig` 用于描述单路相机的帧率、焦距、积分窗口、最大补偿像素、时间偏移、轴向符号和平滑参数。旧版 `calculate_eis_offset(focalX, focalY, targetTimestampNs, halfWindowMs, ...)` 仍保留，便于兼容已有 Demo。
...
int main() {
    ...
    imuConfig.biasCalibMs = 2000;  // 启动时静置标定2秒

    if (!reader.configure(imuConfig)) return -1;
    if (!reader.start()) return -1;

    EisCameraConfig camConfig;
    camConfig.camId = 0;
    camConfig.frameRate = 30.0f;
    camConfig.focalX = 1200.0f;
    camConfig.focalY = 1200.0f;
    camConfig.halfWindowMs = 20;
    camConfig.maxOffsetPixel = 120;
    camConfig.signX = -1.0f;
    camConfig.signY = 1.0f;
    camConfig.enableSmoothing = true;
    camConfig.smoothingAlpha = 0.25f;

    stabilizer.bindReader(&reader);

    uint64_t nowNs = imu_get_time_ns();
    uint64_t frameTimestampNs = nowNs - (uint64_t)camConfig.halfWindowMs * 1000000ULL;

    int32_t offsetX = 0;
    int32_t offsetY = 0;

    bool ok = stabilizer.calculate_eis_offset(camConfig,
                                              frameTimestampNs,
                                              offsetX,
                                              offsetY);
...
* `docs/JITTER-EVALUATION.md`：raw/eis 视频离线帧间位移 RMS 抖动评估说明。
* `docs/PARAMETER-SELECTION.md`：说明 `ImuConfig` 与 `EisCameraConfig` 的参数含义、15FPS/30FPS 双相机推荐配置和调参顺序。
