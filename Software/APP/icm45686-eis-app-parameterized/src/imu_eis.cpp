...
/**************************实现函数********************************************
 *函数原型:     bool Icm45686Reader::configure(const ImuConfig& config)
 *功    能:     按配置设置IMU量程、读取频率和零偏
 *输入参数:     config - IMU全局配置
 *输出参数:     true成功，false失败
 ******************************************************************************/
bool Icm45686Reader::configure(const ImuConfig& config)
{
    ImuConfig newConfig = config;

    if (fd_ < 0 || running_.load()) {
        return false;
    }

    if (!setAccelRange(newConfig.accelRange)) {
        return false;
    }

    if (!setGyroRange(newConfig.gyroRange)) {
        return false;
    }

    if (newConfig.sampleHz <= 0.0f) {
        newConfig.sampleHz = 100.0f;
    }

    /*
     * 零偏标定需要在Reader后台线程启动前完成。
     * 标定期间会主动读取IMU样本，因此要求设备保持静止。
     */
    if (newConfig.enableGyroBiasCalib && newConfig.biasCalibMs > 0) {
        {
            std::lock_guard<std::mutex> lock(configMutex_);
            config_ = newConfig;
            config_.gyroBiasX = 0.0f;
            config_.gyroBiasY = 0.0f;
            config_.gyroBiasZ = 0.0f;
        }

        if (!calibrateGyroBias(newConfig.biasCalibMs, newConfig.sampleHz)) {
            return false;
        }
...
/**************************实现函数********************************************
 *函数原型:     ImuConfig Icm45686Reader::config(void) const
 *功    能:     获取当前IMU配置
 *输入参数:     无
 *输出参数:     IMU配置
 ******************************************************************************/
ImuConfig Icm45686Reader::config() const
{
    std::lock_guard<std::mutex> lock(configMutex_);
    return config_;
}
...
/**************************实现函数********************************************
 *函数原型:     bool Icm45686Reader::calibrateGyroBias(uint32_t durationMs, float sampleHz)
 *功    能:     静置采样计算陀螺仪零偏
 *输入参数:     durationMs - 标定持续时间，sampleHz - 标定采样频率
 *输出参数:     true成功，false失败
 ******************************************************************************/
bool Icm45686Reader::calibrateGyroBias(uint32_t durationMs, float sampleHz)
{
    uint64_t startNs;
    uint64_t durationNs;
    uint64_t periodNs;
    ...
    if (fd_ < 0 || running_.load() || durationMs == 0 || sampleHz <= 0.0f) {
        return false;
    }

    startNs = imu_get_time_ns();
    durationNs = (uint64_t)durationMs * NS_PER_MS;
    periodNs = (uint64_t)((double)NS_PER_SEC / (double)sampleHz);
    nextTime = startNs;

    /*
     * 标定期间通过底层C接口直接读取原始物理量，不调用readSample()。
     * 原因是readSample()会扣除当前config中的bias，而标定需要采集未扣偏的gyro均值。
     */
    while (imu_get_time_ns() - startNs < durationNs) {
    ...
    {
        std::lock_guard<std::mutex> lock(configMutex_);
        config_.gyroBiasX = (float)(sumX / (double)count);
        config_.gyroBiasY = (float)(sumY / (double)count);
        config_.gyroBiasZ = (float)(sumZ / (double)count);
    }
...
bool Icm45686Reader::readSample(ImuSample& sample)
{
    ...
    /*
     * 扣除陀螺仪零偏。
     * 零偏可由静置标定得到，也可由外部配置直接写入。
     */
    sample.gyroX = data.gyro_x - currentConfig.gyroBiasX;
    sample.gyroY = data.gyro_y - currentConfig.gyroBiasY;
    sample.gyroZ = data.gyro_z - currentConfig.gyroBiasZ;
...
/**************************实现函数********************************************
 *函数原型:     bool Icm45686Reader::start(void)
 *功    能:     使用当前ImuConfig中的sampleHz启动后台读取线程
 *输入参数:     无
 *输出参数:     true成功，false失败
 ******************************************************************************/
bool Icm45686Reader::start()
{
    ImuConfig currentConfig = config();
    return start(currentConfig.sampleHz);
}
...
{
    float currentHz = sampleHz_;
    const uint64_t periodNs = (uint64_t)((double)NS_PER_SEC / (double)currentHz);
...
/**************************实现函数********************************************
 *函数原型:     void EisStabilizer::resetSmoothing(int camId)
 *功    能:     清除指定相机的offset平滑状态
 *输入参数:     camId - 摄像头编号
 *输出参数:     无
 ******************************************************************************/
void EisStabilizer::resetSmoothing(int camId)
{
    offsetStates_.erase(camId);
}
...
/**************************实现函数********************************************
 *函数原型:     bool EisStabilizer::calculateWithResolvedParams(...)
 *功    能:     使用已经解析好的目标时间戳和相机配置计算防抖像素补偿量
 *输入参数:     config - 相机EIS配置；targetTimestampNs - 修正后的目标曝光时间戳
 *输出参数:     offsetX/offsetY - 像素补偿量
 *返 回 值:     true成功，false失败
 ******************************************************************************/
bool EisStabilizer::calculateWithResolvedParams(const EisCameraConfig& config,
                                                uint64_t targetTimestampNs,
                                                int32_t& offsetX,
                                                int32_t& offsetY)
{
    uint64_t costStartNs = imu_get_time_ns();
    uint64_t windowNs = (uint64_t)config.halfWindowMs * NS_PER_MS;
    uint64_t startNs;
    uint64_t endNs;
    std::vector<ImuSample> samples;
    float thetaX, thetaY, thetaZ;
    float angleForOffsetX;
    float angleForOffsetY;
    int32_t maxOffset;
    ...
    if (!reader_ || config.focalX <= 0.0f || config.focalY <= 0.0f ||
        config.halfWindowMs == 0 || config.maxOffsetPixel <= 0) {
        return false;
    }
    ...
    /*
     * 默认映射：
     *   thetaY -> offsetX，thetaX -> offsetY。
     * 若安装方向导致X/Y轴交换，可通过swapXY切换为：
     *   thetaX -> offsetX，thetaY -> offsetY。
     * signX/signY用于修正补偿方向，方向错误时可能会导致画面更抖。
     */
    if (config.swapXY) {
        angleForOffsetX = thetaX;
        angleForOffsetY = thetaY;
    } else {
        angleForOffsetX = thetaY;
        angleForOffsetY = thetaX;
    }

    float rawOffsetX = (config.signX >= 0.0f ? 1.0f : -1.0f) * config.focalX * angleForOffsetX;
    float rawOffsetY = (config.signY >= 0.0f ? 1.0f : -1.0f) * config.focalY * angleForOffsetY;

    /*
     * 可选一阶低通平滑：
     *   smooth = alpha * current + (1-alpha) * previous
     * alpha越大响应越快，alpha越小画面更平滑但延迟更大。
     * 平滑状态按camId保存，双相机应使用不同camId避免状态串扰。
     */
    if (config.enableSmoothing && config.smoothingAlpha > 0.0f) {
        float alpha = std::max(0.0f, std::min(1.0f, config.smoothingAlpha));
        OffsetState& state = offsetStates_[config.camId];

        if (!state.valid) {
            state.smoothX = rawOffsetX;
            state.smoothY = rawOffsetY;
            state.valid = true;
        } else {
            state.smoothX = alpha * rawOffsetX + (1.0f - alpha) * state.smoothX;
            state.smoothY = alpha * rawOffsetY + (1.0f - alpha) * state.smoothY;
        }

        rawOffsetX = state.smoothX;
        rawOffsetY = state.smoothY;
    }

    maxOffset = config.maxOffsetPixel > 0 ? config.maxOffsetPixel : maxOffsetPixel_;

    offsetX = (int32_t)lroundf(rawOffsetX);
    offsetY = (int32_t)lroundf(rawOffsetY);

    offsetX = std::max(-maxOffset, std::min(maxOffset, offsetX));
    offsetY = std::max(-maxOffset, std::min(maxOffset, offsetY));
...
/**************************实现函数********************************************
 *函数原型:     bool EisStabilizer::calculate_eis_offset(const EisCameraConfig& config,
 *                                                       uint64_t frameTimestampNs,
 *                                                       int32_t& offsetX, int32_t& offsetY)
 *功    能:     按单路相机配置计算防抖像素补偿量
 *输入参数:     config - 相机EIS配置；frameTimestampNs - 帧中心曝光时间戳
 *输出参数:     offsetX/offsetY - 像素补偿量
 *返 回 值:     true成功，false失败
 ******************************************************************************/
bool EisStabilizer::calculate_eis_offset(const EisCameraConfig& config,
                                         uint64_t frameTimestampNs,
                                         int32_t& offsetX,
                                         int32_t& offsetY)
{
    int64_t correctedTimestampNs = (int64_t)frameTimestampNs +
        (int64_t)((double)config.timeOffsetMs * (double)NS_PER_MS);

    if (correctedTimestampNs < 0) {
        correctedTimestampNs = 0;
    }

    return calculateWithResolvedParams(config, (uint64_t)correctedTimestampNs, offsetX, offsetY);
}
 ...
 *                                                       int32_t& offsetX, int32_t& offsetY)
 *功    能:     兼容旧接口，计算防抖像素补偿量
 *输入参数:     focalX/focalY - 相机焦距；targetTimestampNs - 目标时间戳；halfWindowMs - 半窗口
 *输出参数:     offsetX/offsetY - 像素补偿量
 *返 回 值:     true成功，false失败
 ******************************************************************************/
bool EisStabilizer::calculate_eis_offset(float focalX,
                                         ...
                                         int32_t& offsetY)
{
    EisCameraConfig config;
    config.camId = 0;
    config.focalX = focalX;
    config.focalY = focalY;
    config.halfWindowMs = halfWindowMs;
    config.maxOffsetPixel = maxOffsetPixel_;
    config.signX = signX_;
    config.signY = signY_;
    config.swapXY = false;
    config.enableSmoothing = false;
    config.timeOffsetMs = 0.0f;

    return calculateWithResolvedParams(config, targetTimestampNs, offsetX, offsetY);
}
