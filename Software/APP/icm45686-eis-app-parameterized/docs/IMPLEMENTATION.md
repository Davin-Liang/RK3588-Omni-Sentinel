...
`ImuConfig` 是 IMU 全局配置结构体，主要描述会影响 IMU 数据本身的参数。该结构体通常在 `Icm45686Reader::configure()` 中使用，并且应在 `reader.start()` 前配置完成。

```cpp
struct ImuConfig {
    float sampleHz;              // 应用层IMU读取频率，例如100/200Hz
    ...
    uint32_t biasCalibMs;        // 标定时长，单位ms

    float gyroBiasX;             // X轴零偏，单位rad/s
    float gyroBiasY;
    float gyroBiasZ;
};
```
...
* `sampleHz` 控制用户态后台线程读取 `/dev/icm45686` 的频率；
* `gyroRange` 和 `accelRange` 通过 IOCTL 写入内核驱动；
* `gyroBiasX/Y/Z` 会在 `readSample()` 中被扣除，减少静态积分漂移；
* `enableGyroBiasCalib` 适合 Demo 或设备启动时短时间静置标定；
* 这些参数是设备级参数，双相机系统中通常只需要一份。

### 4.6 EisCameraConfig

`EisCameraConfig` 是单路相机防抖配置结构体。双相机系统中，15FPS 和 30FPS 相机应各自维护一份配置。

```cpp
struct EisCameraConfig {
    int camId;                   // 摄像头编号
    float frameRate;             // 相机帧率
    float focalX;                // 水平焦距，单位pixel
    float focalY;                // 垂直焦距，单位pixel

    uint32_t halfWindowMs;       // IMU积分半窗口
    int32_t maxOffsetPixel;      // 最大像素补偿量

    float timeOffsetMs;          // 相机帧时间戳与IMU时间戳的修正偏移

    float signX;                 // offsetX符号
    float signY;                 // offsetY符号
    bool swapXY;                 // 是否交换X/Y轴映射

    bool enableSmoothing;        // 是否启用offset平滑
    float smoothingAlpha;        // 一阶低通平滑系数
};
```

设计要点：

* `focalX/focalY` 决定角度到像素的换算比例；
* `halfWindowMs` 决定每帧积分使用的 IMU 时间窗口；
* `timeOffsetMs` 用于补偿相机曝光时间戳和 IMU 时间戳之间的偏差；
* `signX/signY/swapXY` 用于适配 IMU 安装方向和图像坐标系；
* `maxOffsetPixel` 必须小于视觉链路可用裁剪余量；
* `enableSmoothing/smoothingAlpha` 用于降低 offset 抖动，但会引入一定延迟；
* `camId` 用于区分不同相机的平滑状态，避免双相机状态串扰。
...
frameTimestampNs
  │
  ├─ 加上 config.timeOffsetMs
  ▼
targetTimestampNs
  │
  ├─ 查询 [target-halfWindow, target+halfWindow] 内 IMU 样本
  ▼
gyro 梯形积分
  │
  ├─ 根据 config.swapXY 选择 thetaX/thetaY 映射
  ├─ 根据 config.signX/signY 修正补偿方向
  ├─ 根据 config.focalX/focalY 转换为像素
  ├─ 根据 config.smoothingAlpha 做可选平滑
  └─ 根据 config.maxOffsetPixel 做限幅
  ▼
offsetX / offsetY
```

这种接口的好处是：15FPS 和 30FPS 相机可分别使用不同的 `halfWindowMs`、`timeOffsetMs`、`signX/signY` 和 `smoothingAlpha`，但共享同一个 IMU Reader。
