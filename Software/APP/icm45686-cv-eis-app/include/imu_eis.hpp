...
/* IMU辅助状态：用于“视觉为主 + IMU辅助”防抖方案。
 * 视觉模块不再强依赖 IMU 直接输出 offset，而是使用这些状态判断震动强度，
 * 进而动态调整轨迹平滑强度或在视觉估计失败时做可靠性判断。
 */
struct ImuAssistState {
    ...
    float accelNorm;       /* 当前加速度模长，单位 m/s² */
    float gyroNorm;        /* 当前角速度模长，单位 rad/s */
    float gyroRms;         /* 最近窗口内角速度 RMS，单位 rad/s */
    ...
    ImuAssistState()
        : timestampNs(0),
          ...
          accelNorm(0.0f), gyroNorm(0.0f), gyroRms(0.0f),
          vibrationLevel(1)
    {
    }
};
...
public:
    ...
    /**************************实现函数****************************************
     *函数原型:     bool getAssistState(ImuAssistState& state, uint32_t windowMs)
     *功    能:     获取“视觉为主 + IMU辅助”方案所需的震动状态。
     *              该接口不会直接输出防抖 offset，而是输出 gyroNorm/gyroRms/震动等级。
     *输入参数:     windowMs - 统计 gyroRms 的时间窗口，默认200ms
     *输出参数:     state - IMU辅助状态
     *************************************************************************/
    bool getAssistState(ImuAssistState& state, uint32_t windowMs = 200) const;
