...
本版本新增 `include/vision_eis.hpp`、`src/vision_eis.cpp` 和 `src/vision_eis_offline_demo.cpp`，支持“视觉为主 + IMU 辅助”的电子防抖方案。

核心变化：

- LK 光流 + RANSAC 估计画面帧间运动，不需要训练模型；
- IMU 不再直接决定 `offsetX/offsetY`，而是输出 `gyroNorm / gyroRms / vibrationLevel` 辅助调节平滑强度；
- PC 端可用 `icm45686_vision_eis_offline` 从 `raw.mp4` 离线生成 `eis_visual.mp4`；
- 仍然使用 `icm45686_jitter_eval raw.mp4 eis_visual.mp4` 评价 Translation RMS 抖动降低率。
...
cd build_pc
cmake ..
make -j4 icm45686_vision_eis_offline icm45686_jitter_eval
./icm45686_vision_eis_offline raw.mp4 eis_visual.mp4
./icm45686_jitter_eval raw.mp4 eis_visual.mp4
```

RK3588 板端交叉编译时，如果 sysroot 中没有 OpenCV，会跳过视觉离线工具，不影响 `icm45686_app` 和 `icm45686_eis_demo`。

## IMU Assist for Visual EIS

本版本中，ICM45686 模块主要作为视觉 EIS 的辅助输入。新增 `ImuAssistState` 与 `Icm45686Reader::getAssistState()`，用于输出：
...
最终 `offsetX / offsetY` 由 `sentinel-visioner` 中的视觉 EIS 模块根据相机画面估计得到，IMU 只负责辅助判断震动强度和调节平滑参数。
