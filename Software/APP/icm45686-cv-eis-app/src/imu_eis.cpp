...
/**************************实现函数********************************************
 *函数原型:     bool Icm45686Reader::getAssistState(ImuAssistState& state,
 *                                                  uint32_t windowMs) const
 *功    能:     生成 IMU 辅助状态，用于视觉为主的 EIS 方案
 *说    明:     该接口只描述当前震动强度，不直接决定 offsetX/offsetY，
 *              从而避免纯 IMU 防抖中 IMU-相机坐标系难标定的问题。
 ******************************************************************************/
bool Icm45686Reader::getAssistState(ImuAssistState& state, uint32_t windowMs) const
{
    ImuSample latest;
    if (!getLatestSample(latest)) {
        return false;
    }

    state.timestampNs = latest.timestampNs;
    state.accelX = latest.accelX;
    state.accelY = latest.accelY;
    state.accelZ = latest.accelZ;
    state.gyroX = latest.gyroX;
    state.gyroY = latest.gyroY;
    state.gyroZ = latest.gyroZ;

    state.accelNorm = std::sqrt(latest.accelX * latest.accelX +
                                latest.accelY * latest.accelY +
                                latest.accelZ * latest.accelZ);
    state.gyroNorm = std::sqrt(latest.gyroX * latest.gyroX +
                               latest.gyroY * latest.gyroY +
                               latest.gyroZ * latest.gyroZ);

    uint64_t windowNs = (uint64_t)windowMs * NS_PER_MS;
    uint64_t startNs = latest.timestampNs > windowNs ? latest.timestampNs - windowNs : 0;

    std::vector<ImuSample> samples;
    if (!getSamplesBetween(startNs, latest.timestampNs, samples) || samples.empty()) {
        state.gyroRms = state.gyroNorm;
    } else {
        double sum2 = 0.0;
        for (size_t i = 0; i < samples.size(); ++i) {
            double gx = samples[i].gyroX;
            double gy = samples[i].gyroY;
            double gz = samples[i].gyroZ;
            double norm = std::sqrt(gx * gx + gy * gy + gz * gz);
            sum2 += norm * norm;
        }
        state.gyroRms = (float)std::sqrt(sum2 / (double)samples.size());
    }

    /* 经验阈值，后续可根据实际设备震动日志调整。 */
