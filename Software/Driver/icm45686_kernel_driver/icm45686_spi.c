...
/*
 * 调试阶段建议使用较低SPI频率，避免因为线长、接线、时序不稳定导致WHO_AM_I读取失败。
 * 如果确认硬件接线稳定后，可以再提高到10000000。
 */
#define ICM45686_SPI_DEBUG_SPEED_HZ  1000000
                              ...
                              uint8_t *buf, uint32_t len)
{
    ...
    uint8_t tx_buf = reg | 0x80;  /* SPI读操作：寄存器地址最高位置1 */
    int ret;

    ret = spi_write_then_read(spi, &tx_buf, 1, buf, len);
    if (ret < 0) {
        dev_err(&spi->dev, "SPI read failed, reg=0x%02x, ret=%d\n", reg, ret);
        return ret;
    }
                               ...
                               const uint8_t *buf, uint32_t len)
{
    ...
    tx_buf[0] = reg & 0x7F;  /* SPI写操作：寄存器地址最高位置0 */
    memcpy(&tx_buf[1], buf, len);

    ret = spi_write(spi, tx_buf, len + 1);
    if (ret < 0)
        dev_err(&spi->dev, "SPI write failed, reg=0x%02x, ret=%d\n", reg, ret);
...
static int icm45686_create_char_device(struct icm45686_data *data)
{
    ...
    if (IS_ERR(data->class)) {
        ...
        dev_err(&data->spi->dev, "Failed to create class, ret=%d\n", ret);
        cdev_del(&data->cdev);
        unregister_chrdev_region(data->devno, 1);
        return ret;
    }
    ...
    /* 创建设备节点 */
    dev_ret = device_create(data->class, NULL, data->devno, NULL, DEVICE_NAME);
    if (IS_ERR(dev_ret)) {
        ret = PTR_ERR(dev_ret);
        dev_err(&data->spi->dev, "Failed to create device, ret=%d\n", ret);
...
/**************************实现函数********************************************
 *函数原型:     static int icm45686_hw_reset(struct icm45686_data *data)
 *功　　能:     执行ICM45686硬件复位
 *输入参数:     data - 设备数据结构
 *输出参数:     0成功，负数失败
 ******************************************************************************/
static int icm45686_hw_reset(struct icm45686_data *data)
{
    struct spi_device *spi = data->spi;

    /*
     * reset-gpios为可选配置。
     * 如果当前硬件没有连接RESET/RST引脚，则跳过硬件复位，仅依靠芯片上电状态。
     */
    if (!data->reset_gpio) {
        dev_warn(&spi->dev, "No reset GPIO defined, skip hardware reset\n");
        msleep(50);
        return 0;
    }
    ...
    /*
     * 注意：
     * 设备树中若配置 GPIO_ACTIVE_LOW，则gpiod_set_value()使用的是逻辑值。
     * 逻辑1表示assert reset，物理上拉低复位脚；
     * 逻辑0表示deassert reset，物理上释放复位脚。
     */
    ...
    dev_info(&spi->dev, "Hardware reset finished\n");
    return 0;
}
...
static int icm45686_probe(struct spi_device *spi)
{
    ...
    /*
     * 配置SPI通信参数。
     * 调试阶段先使用SPI_MODE_0和1MHz，便于排查接线、片选、MISO/MOSI和时序问题。
     * 如果确认ICM45686硬件要求SPI_MODE_3，可以将SPI_MODE_0改回SPI_MODE_3。
     */
    spi->mode = SPI_MODE_3;
    spi->bits_per_word = 8;
    spi->max_speed_hz = ICM45686_SPI_DEBUG_SPEED_HZ;

    ret = spi_setup(spi);
    if (ret) {
        dev_err(&spi->dev, "spi_setup failed, ret=%d\n", ret);
        return ret;
    }

    dev_info(&spi->dev, "SPI configured: mode=%u, bits_per_word=%u, max_speed_hz=%u\n",
             spi->mode, spi->bits_per_word, spi->max_speed_hz);

    /*
     * 初始化RESET引脚。
     * RESET引脚为可选项，当前只接SPI和电源时也允许继续probe。
     */
    data->reset_gpio = devm_gpiod_get_optional(&spi->dev, "reset", GPIOD_OUT_LOW);
    ...
    /*
     * 初始化INT引脚。
     * 这里的interrupt-gpio只是可选GPIO描述符；如果设备树只配置interrupts属性，
     * 此处可能返回NULL，不影响基础SPI通信和WHO_AM_I读取。
     */
    data->int_gpio = devm_gpiod_get_optional(&spi->dev, "interrupt", GPIOD_IN);
    if (IS_ERR(data->int_gpio)) {
        dev_err(&spi->dev, "Failed to get interrupt GPIO: %ld\n",
                PTR_ERR(data->int_gpio));
        return PTR_ERR(data->int_gpio);
    }

    /* 硬件复位 */
    ret = icm45686_hw_reset(data);
    if (ret)
        return ret;
    ...
    if (ret) {
        dev_err(&spi->dev, "Failed to initialize ICM45686, ret=%d\n", ret);
        return ret;
    }
    ...
    if (ret) {
        dev_err(&spi->dev, "Failed to create char device, ret=%d\n", ret);
        return ret;
    }
